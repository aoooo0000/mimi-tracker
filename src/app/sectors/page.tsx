...
export default async function SectorsPage() {
  ...
  const sectors = Object.entries(grouped).map(([sector, items]) => {
    const returns = items.map((r) => (((r.currentPrice ?? r.recommendPrice) - r.recommendPrice) / r.recommendPrice) * 100);
        ...
        {sectors.map((s) => (
          <section key={s.sector} className="card">
            <div className="mb-3 flex items-center justify-between">
              ...
              <div className="text-sm text-slate-400">命中率 {s.hitRate.toFixed(1)}% · 平均 {s.avgReturn.toFixed(2)}%</div>
            </div>
